...
exports.requestMomoPayment = functions.https.onCall(async (data, context) => {
  ...
  const { orderId, phone, amountUsd } = data;

  if (!orderId || !phone || !amountUsd) {
    throw new functions.https.HttpsError("invalid-argument", "Missing orderId, phone, or amountUsd.");
  }
  ...
  try {
    ...
    await axios.post(
      `${MOMO_BASE_URL}/collection/v1_0/requesttopay`,
      {
        ...
        payerMessage: "Payment for Waterside order",
    ...
    console.error("MoMo request failed:", error.response ? error.response.data : error.message);
    throw new functions.https.HttpsError("internal", "Could not process payment request.");
  }
});
...
exports.checkMomoPaymentStatus = functions.https.onCall(async (data, context) => {
  ...
  if (!referenceId) {
    throw new functions.https.HttpsError("failed-precondition", "No payment request found for this order.");
  }
  ...
  try {
    ...
    throw new functions.https.HttpsError("internal", "Could not check payment status.");
  }
});